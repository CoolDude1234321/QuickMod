...
function getenemylimitbreak(_enemy){
  ...
  if(_enemy == "Magician") return "Teleport";
  if(_enemy == "Gardener") return "Burrower";
  if(_enemy == "Robobot") return "Plasma Gun";
  if(_enemy == "Wolf Puppy") return "Fury";
  if(_enemy == "Slime") return "Poison Arrow";
  if(_enemy == "Marshmallow") return "Oh No!";
  if(_enemy == "Wizard") return "Infinity Mirror";
  if(_enemy == "Dryad") return "Recovery";
  if(_enemy == "Sneezy") return "Accelerate";
  if(_enemy == "Baby Squid") return "Oh No!";
  if(_enemy == "Pirate") return "Hook";
  if(_enemy == "Handyman") return "Focus"; //:smore:
  if(_enemy == "Haunted Jar") return "Cackle";
  if(_enemy == "Vacuum") return "Blood Sip";
  if(_enemy == "Drain Monster") return "Burrower";
  if(_enemy == "Cactus") return "Fury";
  if(_enemy == "Fireman") return "Heat Ray";
  if(_enemy == "Yeti") return "Unlucky Roll";
  if(_enemy == "Wicker Man") return "Hellfire";
  if(_enemy == "Rhino Beetle") return "Fury";
  if(_enemy == "Snowman") return "Arctic Storm";
  if(_enemy == "Rose") return "Needle";
  if(_enemy == "Wisp") return "Firebomb";
  if(_enemy == "Dire Wolf") return "Dramatic Pause";
  if(_enemy == "Skeleton") return "Cackle";
  if(_enemy == "Rat King") return "Overload";
  if(_enemy == "Gargoyle") return "Meganudge";
  if(_enemy == "Banshee") return "Scream";
  if(_enemy == "Singer") return "Scream";
...
if(isplayer){
  if(enemylist.indexOf(transforminto.split("_Old")[0]) != -1){
    //Player, transforming into an enemy
    var altlayout = false;
    if(transforminto == "Robobot_Old"){
      trace(1);
      altlayout = true;
      transforminto = "Robobot";
    }
    ...
    for (i in 0 ... f.equipment.length){
      if(permanenttransform){
        ...
        if(!(f.equipment[i] == f.getskillcard() && f.usecpuinsteadofdice) || f.roll_jackpot > 0){
          f.equipment[i].ready = false;
          f.equipment[i].show = false;
    ...
    if(altlayout && transforminto == "Robobot") newequipment = ["Ray Gun"];
    if((!f.usecpuinsteadofdice || f.roll_jackpot > 0) && transforminto == "Buster") newequipment.remove("Fire Breath");
    ...
    for (i in 0 ... f.equipment.length){
      if(permanenttransform){
        f.equipment[i].dispose();
      }else if(transforminto != "Robot" || !altlayout){
        f.equipment[i].ready = false;
        f.equipment[i].show = false;
        f.equipment[i].y = -1000;
      }
    }f.getskillcard().onceperbattle = true;
    f.getskillcard().usedthisbattle = true;
    ...
    if(transforminto == "Warrior"){
      ...
      if(!altlayout){
        ...
        f.changelimitbreak("Optimise"); //todo add optimise
      }
      ...
      if(!altlayout){
        ...
        f.changelimitbreak("Against All Odds");
      }
    }
    if(altlayout && transforminto == "Robot"){
      f.changelimitbreak("Jackpot");
      f.roll_target = 5 + target.dice*2;
      f.bonusdice = -f.dice;
      f.usecpuinsteadofdice = true;
    }
    if(altlayout && transforminto == "Jester"){
